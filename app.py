...
BROKEN_HASH =0  # хеширование работает - 0,  хеширование сломалось - 1
DONT_CREATE_HASH =0  # создать хеши - 0,  не создавать хеши - 1
...
class App:
    ...
    def SignIn(self):
        ...
        if not BROKEN_HASH:
            tPassword_hash = generate_hashM2(tPassword) #захешированный пароль
        if BROKEN_HASH:
            tPassword_hash = tPassword
        with open("data.json") as file:
            self.data = json.load(file)

        for item in self.data["users"]:
            if item["password"] == tPassword_hash and item["login"] == tLogin:
                if item["blocked"]:
    ...
    def change_password(self):
        ...
        if not BROKEN_HASH:
            old_password_hash = generate_hashM2(old_password)
        else:
            old_password_hash = old_password

        if old_password_hash == self.current_user["password"]:
            ...
            if (self.current_user["restrictions"]):
                ...
                if has_latin and has_operators:
                    self.current_user["password"] = generate_hashM2(new_password)
                else:
                     messagebox.showerror("Смена пароля", "Пароль должен содержать кириллицу и латиницу")
                     return
            else:
                if not DONT_CREATE_HASH:
                    self.current_user["password"] = generate_hashM2(new_password)
                else:
